...
1. **消融跑批脚本从 8 格改成 2×3（6 格）**：`scripts/run_stance_ablation.py`
   - 现：base/lora × {无, aligned, neutral, opposed} = 8 格
   - 改：base/lora × {无, aligned, opposed} = 6 格，去掉 neutral
2. **管理面板田字格从 8 格改成 2×3**：`rag_console/static/`（app.js/index.html）
   - 现：上排无 RAG 2 格 + 下排三档（三联按钮）
   - 改：下排两档（正向/负向，两个按钮），去掉中性列
3. **中性档处置（需你拍板）**：
   - 方案 A（推荐）：**保留 neutral 数据、本实验不调用**。零成本，且 neutral/opposed 是同题配对，
     删了会破坏配对的完整性（未来若想补"中性档"对比还能用）。
   - 方案 B：物理删除 neutral（需重建库表 + FAISS 索引）。仅当你们确定彻底不要时才做。
...
> 注：第 1 步的"改脚本/田字格"我随时可以执行，等你确认中性档走方案 A 还是 B 后一并做。
